...
window.mapsData = {
	...
	// ===== Season 14 (current) =====
	// Season 14 launches with the Season 13 clear times. Edit this array for Season 14.
	s14: [
		// Tier 3
...
window.mapsDataHelpers = {
	...
	// Build tier-grouped Solo/MF dataset for S14 in `{tier, maps:[{name,slug,top,good,decent,elems}]}` shape.
	// `variant` is 'fullClear' (default), 'eliteSkip', or 'ear'.
	// 'ear' is derived from mfFullClear * 1.75 (Ear runs spend 75% longer per mob since mobs drop more loot).
	buildS14MfData: function (variant) {
		var fmt = this.fmtMin;
		var field = variant === 'eliteSkip' ? 'mfEliteSkip' : 'mfFullClear';
		var earMultiplier = (variant === 'ear') ? 1.75 : 1;
		var byTier = { 3: [], 2: [], 1: [] };
		window.mapsData.s14.forEach(function (m) {
			var raw = m[field];
			var arr = (raw && earMultiplier !== 1)
				? [raw[0] * earMultiplier, raw[1] * earMultiplier, raw[2] * earMultiplier]
				: raw;
	...
	// Build tier-grouped EXP dataset for S14 in `{tier, maps:[[name,top,good,decent]]}` shape.
	// `variant` is 'exp98' or 'expNone'.
	buildS14Exp: function (variant) {
		var byTier = { 3: [], 2: [], 1: [] };
		window.mapsData.s14.forEach(function (m) {
			var arr = m[variant];
			byTier[m.tier].push(arr
	...
	// Element-resistance lookup keyed by map name (S14).
	getS14Elems: function () {
		var out = {};
		window.mapsData.s14.forEach(function (m) { out[m.name] = m.elems; });
		return out;
	}
};
